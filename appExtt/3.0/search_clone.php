 ...
 *
 */
global $phpbb_root_path, $phpEx, $cache, $searchResults, $mobiquo_config, $user, $auth, $db, $template, $request_method, $total_match_count;
...
switch ($search_id)
{
	...
	case 'egosearch':
		...
		{
		    return trigger_error('LOGIN_EXPLAIN_EGOSEARCH');
	...
	case 'newposts':
		if ($user->data['user_id'] == ANONYMOUS)
		{
		    return trigger_error('LOGIN_EXPLAIN_NEWPOSTS');
...
if ($search_id != 'subscribedtopics')
{
    ...
    {
    	$template->assign_var('S_NO_SEARCH', true);
    	return trigger_error('NO_SEARCH');
    }
    ...
    {
    	$template->assign_var('S_NO_SEARCH', true);
    	return trigger_error('NO_SEARCH_TIME');
    }
}
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	else if ($author)
	{
		...
		{
			return trigger_error(sprintf($user->lang['TOO_FEW_AUTHOR_CHARS'], $config['min_search_author_chars']));
		}
		...
		{
			return trigger_error('NO_SEARCH_RESULTS');
		}
	}
	...
	{
		return trigger_error('NO_SUCH_SEARCH_MODULE');
	}
	...
	if ($keywords)
	{
		...
		if (!$correct_query || (empty($search->search_query) && !sizeof($author_id_ary) && !$search_id))
		{
			...
			return trigger_error($ignored . sprintf($user->lang['NO_KEYWORDS'], $search->word_length['min'], $search->word_length['max']));
		}
	}
