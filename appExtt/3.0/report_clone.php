 ...
 *
 */
global $db,$auth,$user,$phpbb_dispatcher, $phpbb_notifications, $phpbb_container, $template;
/**
 * @ignore
 */
if(!defined('IN_PHPBB'))
...
{
	return trigger_error('NO_POST_SELECTED');
}
...
if ($post_id)
{
	...
	if (!$report_data)
	{
		return trigger_error('POST_NOT_EXIST');
	}
	...
	if (!$forum_data)
	{
		return trigger_error('FORUM_NOT_EXIST');
	}
...
else
{
	...
	{
		$user->add_lang('ucp');
		return trigger_error('NO_MESSAGE');
	}
