...
/**
 *  TAPATALK - this file was a copy of the original file viewtopic.php
 *
 *
 ...
 *
 */
global $db, $phpbb_root_path, $phpEx, $user, $phpbb_container, $phpbb_dispatcher, $pagination, $auth, $attachment_by_id, $support_post_thanks, $topic_data, $total_posts, $can_subscribe, $post_data;
...
$onlyOnePost = request_var('onlyOnePost', 0);
...
if ($post_id)
{
$sql_array['SELECT'] .= ', p.post_visibility, p.post_time, p.post_id, p.poster_id';
...
if ($post_id && $topic_id)
{
//TAPATALK REMOVED -> redirect(append_sid("{$phpbb_root_path}viewtopic.$phpEx", "t=$topic_id" . (($forum_id)S_ONLINE ? "&amp;f=$forum_id" : '')));
}
...
if ($topic_id)
{
//TAPATALK REMOVED -> redirect(append_sid("{$phpbb_root_path}viewtopic.$phpEx", "t=$topic_id" . (($forum_id) ? "&amp;f=$forum_id" : '')));
}
...
//login_box('', getSystemString('LOGIN_VIEWFORUM'));
}
...
/* TAPATALK REMOVED
  ...
  {
  login_box(build_url('e') . '#unread', getSystemString('LOGIN_NOTIFY_TOPIC'));
  }
 */
...
if ($post_id)
{
$start = floor(($topic_data['prev_posts']) / $limit) * $limit;
}
...
$limit_days = array(0 => getSystemString('ALL_POSTS'), 1 => getSystemString('1_DAY'), 7 => getSystemString('7_DAYS'), 14 => getSystemString('2_WEEKS'), 30 => getSystemString('1_MONTH'), 90 => getSystemString('3_MONTHS'), 180 => getSystemString('6_MONTHS'), 365 => getSystemString('1_YEAR'));

$sort_by_text = array('a' => getSystemString('AUTHOR'), 't' => getSystemString('POST_TIME'), 's' => getSystemString('SUBJECT'));
...
$start = $pagination->validate_start($start, $limit, $total_posts);
...
/* TAPATALK REMOVED
  ...
  $message = (($topic_data['bookmarked']) ? getSystemString('BOOKMARK_REMOVED'] : $user->lang['BOOKMARK_ADDED'));
  ...
  {
  $message = getSystemString('BOOKMARK_ERR');
 ...
 *
  ...
  $pagination->generate_template_pagination($base_url, 'pagination', 'start', $total_posts, $limit, $start);
  ...
  'MODERATORS'	=> (isset($forum_moderators[$forum_id]) && sizeof($forum_moderators[$forum_id])) ? implode(getSystemString('COMMA_SEPARATOR'), $forum_moderators[$forum_id]) : '',
  ...
  'S_BOOKMARK_TOPIC'		=> ($user->data['is_registered'] && $config['allow_bookmarks'] && $topic_data['bookmarked']) ? getSystemString('BOOKMARK_TOPIC_REMOVE'] : $user->lang['BOOKMARK_TOPIC'),
  'S_BOOKMARK_TOGGLE'		=> (!$user->data['is_registered'] || !$config['allow_bookmarks'] || !$topic_data['bookmarked']) ? getSystemString('BOOKMARK_TOPIC_REMOVE'] : $user->lang['BOOKMARK_TOPIC'),
  ...
  $message = $user->lang[$message] . '<br /><br />' . sprintf(getSystemString('RETURN_TOPIC'), '<a href="' . $redirect_url . '">', '</a>');
  trigger_error($message);
  }
  ...
  $redirect_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start"));
  $message = getSystemString('VOTE_SUBMITTED'] . '<br /><br />' . sprintf($user->lang['RETURN_TOPIC'), '<a href="' . $redirect_url . '">', '</a>');
  ...
  $data = array(
  'NO_VOTES'			=> getSystemString('NO_VOTES'),
  'success'			=> true,
  ...
  'L_POLL_LENGTH'		=> ($topic_data['poll_length']) ? sprintf($user->lang[($poll_end > time()) ? 'POLL_RUN_TILL' : 'POLL_ENDED_AT'], mobiquo_format_date($poll_end)) : '',
...
$sql = 'SELECT p.post_id
	...
	WHERE p.topic_id = $topic_id
		AND " . ($onlyOnePost ? '1=1' : $phpbb_content_visibility->get_visibility_sql('post', $forum_id, 'p.')) . "
...
return trigger_error("Function 'insert_thanks' does not exist!");
}
}

if (function_exists('naguissa\thanksforposts\core\array_all_thanks')) {
naguissa\thanksforposts\core\array_all_thanks($post_list);
$support_post_thanks = true;
} elseif (function_exists('naguissa\ThanksForPosts\core\array_all_thanks')) {
...
$message = tapatalk_process_bbcode($row['post_text'], $row['bbcode_uid']);
...
$l_edited_by = $user->lang('EDITED_TIMES_TOTAL', (int) $row['post_edit_count'], $display_username, $row['post_edit_time']);
}
else
{
...
{
$l_deleted_message = $user->lang('POST_DELETED_BY_REASON', $display_postername, $display_username, $row['post_delete_time'], $row['post_delete_reason']);
}
else
{
$l_deleted_message = $user->lang('POST_DELETED_BY', $display_postername, $display_username, $row['post_delete_time']);
}
$l_deleted_by = $user->lang('DELETED_INFORMATION', $display_username, $row['post_delete_time']);
}
else
{
...
$l_bumped_by = sprintf(getSystemString('BUMPED_BY'), $user_cache[$topic_data['topic_bumper']]['username'], $topic_data['topic_last_post_time']);
}
else
{
$l_bumped_by = '';
...
'POST_AUTHOR_FULL' => ($poster_id != ANONYMOUS) ? $user_cache[$poster_id]['author_full'] : get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']),
 ...
 'L_IGNORE_POST' => ($row['foe']) ? sprintf(getSystemString('POST_BY_FOE'), get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username'])) : '',
...
array(
'ID' => 'pm',
 'NAME' => getSystemString('SEND_PRIVATE_MESSAGE'),
 'U_CONTACT' => $u_pm,
 ),
 array(
'ID' => 'email',
 'NAME' => getSystemString('SEND_EMAIL'),
 'U_CONTACT' => $user_cache[$poster_id]['email'],
 ),
 array(
'ID' => 'jabber',
 'NAME' => getSystemString('JABBER'),
...
/* //// If there are absolutely no more unread posts in this forum
  ...
  {
  $last_page = ((floor($start / $limit) + 1) == max(ceil($total_posts / $config['posts_per_page']), 1)) ? true : false;
...
//if (!request_var('f', 0))
//{
//    overwriteRequestParam('f', $forum_id);
//}
//// We need to do the same with the topic_id. See #53025.
//if (!request_var('t', 0) && !empty($topic_id))
//{
//    overwriteRequestParam('t', $topic_id);
//}
//$page_title = $topic_data['topic_title'] . ($start ? ' - ' . sprintf(getSystemString('PAGE_TITLE_NUMBER'], $pagination->get_on_page($config['posts_per_page'), $start)) : '');
///**
