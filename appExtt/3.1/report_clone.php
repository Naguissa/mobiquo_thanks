<?php
/**
 *  TAPATALK - this file was a copy of the original file report.php
 *
 *
...
global $db,$auth,$user,$phpbb_dispatcher, $phpbb_notifications, $phpbb_container,$config,$template;
...
if ($post_id)
{
	...
	{
		$message = getSystemString('ALREADY_REPORTED');
		$message .= '<br /><br />' . sprintf(getSystemString('RETURN_TOPIC'), '<a href="' . $redirect_url . '">', '</a>');
		$message .= '<br /><br />' . sprintf(getSystemString('RETURN_FORUM'), '<a href="' . $return_forum_url . '">', '</a>');
	}
}
else
{
	...
	{
		$message = getSystemString('ALREADY_REPORTED_PM');
		$message .= '<br /><br />' . sprintf(getSystemString('RETURN_PM'), '<a href="' . $redirect_url . '">', '</a>');
...
if ($submit && $reason_id)
{
	...
	if (!sizeof($error))
	{
		...
		if ($post_id)
		{
			...
			$lang_return = getSystemString('RETURN_TOPIC');
			$lang_success = getSystemString('POST_REPORTED_SUCCESS');
		...
		else
		{
			...
			$lang_return = getSystemString('RETURN_PM');
			$lang_success = getSystemString('PM_REPORTED_SUCCESS');
...
$page_title = ($pm_id) ? getSystemString('REPORT_MESSAGE') : getSystemString('REPORT_POST');
