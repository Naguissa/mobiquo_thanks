 ...
 *
 */
global $db, $phpbb_root_path, $phpEx, $user, $post_data, $post_list, $auth, $db, $template, $attachment_by_id, $support_post_thanks, $topic_data, $total_posts, $can_subscribe, $post_data;
...
if ($view && !$post_id) {
	...
	if ($view == 'unread') {
		...
		if (!$row) {
			...
			return trigger_error(($view == 'next') ? 'NO_NEWER_TOPICS' : 'NO_OLDER_TOPICS');
		} else {
...
if (!$auth->acl_get('f_read', $forum_id) && $topic_data['topic_type'] != POST_GLOBAL) {
	if ($user->data['user_id'] != ANONYMOUS) {
		return trigger_error('SORRY_AUTH_READ');
	}

	return trigger_error('LOGIN_VIEWFORUM');
...
if ($topic_data['forum_password'] && !check_forum_password($forum_id)) {
	return trigger_error('LOGIN_FORUM');
...
/*
  ...
  if ($user->data['user_id'] == ANONYMOUS)
  {
  return trigger_error('LOGIN_NOTIFY_TOPIC');
...
/*
  ...
  meta_refresh(5, $redirect_url);
  return trigger_error($user->lang['VOTE_SUBMITTED']);
  //return trigger_error($user->lang['VOTE_SUBMITTED'] . '<br /><br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $redirect_url . '">', '</a>'));
  }
  ...
  'L_POLL_LENGTH'		=> ($topic_data['poll_length']) ? sprintf($user->lang[($poll_end > time()) ? 'POLL_RUN_TILL' : 'POLL_ENDED_AT'], $poll_end) : '',
...
$sql = 'SELECT p.post_id
	...
	WHERE p.topic_id = $topic_id
		" . ($onlyOnePost ? 'AND 1=1' : ((!$auth->acl_get('m_approve', $forum_id)) ? 'AND p.post_approved = 1' : '')) . "
...
if (!sizeof($post_list)) {
	if ($sort_days) {
		return trigger_error('NO_POSTS_TIME_FRAME');
	} else {
		return trigger_error('NO_TOPIC');
	}
}
...
if (file_exists($phpbb_root_path . 'includes/functions_thanks.' . $phpEx) && isset($config['remove_thanks'])) {
	...
	} elseif (function_exists('gfksx\ThanksForPosts\core\array_all_thanks')) {
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i) {
	...
	$message = tapatalk_process_bbcode($message, $row['bbcode_uid']);
	...
	if (($row['post_edit_count'] && $config['display_last_edited']) || $row['post_edit_reason']) {
		...
		if ($row['post_edit_reason']) {
			...
			$l_edited_by = sprintf($l_edit_time_total, $display_username, $row['post_edit_time'], $row['post_edit_count']);
		} else {
	...
	if ($topic_data['topic_bumped'] && $row['post_id'] == $topic_data['topic_last_post_id'] && isset($user_cache[$topic_data['topic_bumper']])) {
		...
		$l_bumped_by = sprintf($user->lang['BUMPED_BY'], $user_cache[$topic_data['topic_bumper']]['username'], $topic_data['topic_last_post_time']);
	} else {
		$l_bumped_by = '';
	}
	...
	if (function_exists('naguissa\thanksforposts\core\output_thanks') && $support_post_thanks) {
		naguissa\thanksforposts\core\output_thanks($row['user_id']);
...
// *************** MOVED TO MbqWrEtForumTopic->addForumTopicViewNum ***************//
