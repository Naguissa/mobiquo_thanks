...
$onlyOnePost = $request->variable('onlyOnePost', 0);
...
// Parse the message and subject
	...
	$message = generate_text_for_edit($row['post_text'], $row['bbcode_uid'], $row['bbcode_bitfield']);
...
// Admins and mods are bound to the above rules too...
		$l_bumped_by = sprintf(getSystemString('BUMPED_BY'), $user_cache[$topic_data['topic_bumper']]['username'], $user->format_date($topic_data['topic_last_post_time'], false, true));
	} else {
		$l_bumped_by = '';
	}
...
// tapatalk add for thanks support
	if ($support_post_thanks && $thanksHelper) {
		$thanksHelper->output_thanks($row['user_id'], $row['post_row'], $row['row'], $row['topic_data'], $forum_id);
	}
