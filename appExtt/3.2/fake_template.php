...
class fake_template extends \phpbb\template\base
{
    public $pagination;
    public function getTemplateVars()
    {
        $vars = $this->context->get_data_ref();
        $vars = $vars['.'][0];
        return $vars;
    }
    public function getTemplateVar($varname)
    {
        $var = $this->context->get_data_ref();
    ...
    public function __construct()
	{
        ...
        $this->pagination = new fake_pagination($this, $user, $phpbb_dispatcher);
        ...
        overwriteRequestParam('confirm', getSystemString('YES'), \phpbb\request\request_interface::POST);
...
class fake_pagination extends \phpbb\pagination
{
     ...
     */
	public function __construct(\phpbb\template\template $template, \phpbb\user $user, \phpbb\event\dispatcher_interface $phpbb_dispatcher)
	{
